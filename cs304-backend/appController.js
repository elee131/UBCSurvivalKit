...
router.get("/utils-at-building", async (req, res) => {
  ...
  const results = await appService.findUtilsAtBuilding(
    buildingCode,
    wrClicked,
    mClicked,
    wfClicked
...
router.post("/log-in", async (req, res) => {
  try {
    ...
    if (email === undefined || password === undefined || password === "") {
      return res
        .status(400)
        .json({ success: false, message: "Missing userID or password" });
    }
...
router.get("/requested-utilities", async (req, res) => {
  ...
  const tableContent = await appService.fetchRequestedUtils(
    wrClicked,
    mClicked,
    wfClicked
  );
...
router.get("/requested-utilities-simple", async (req, res) => {
  const { wrClicked, mClicked, wfClicked } = req.query;
  const tableContent = await appService.fetchRequestedUtilsSimple(
    wrClicked,
    mClicked,
    wfClicked
  );
...
router.post("/insert-waterfountain", async (req, res) => {
  ...
  if (newUtilID === undefined || newUtilID === null) {
    return res
      .status(400)
      .json({ success: false, message: "Failed to find suitable utilID" });
  }
...
router.post("/insert-location", async (req, res) => {
  ...
  if (newLocationID === undefined || newLocationID === null) {
    return res
      .status(400)
      .json({ success: false, message: "Failed to find suitable locationID" });
  }
...
router.post("/new-user", async (req, res) => {
  ...
  if (currMaxUserID === null || currMaxUserID === undefined) {
    res
      .status(400)
      .json({ status: false, message: "failed to find suitable UID" });
  } else {
    newUID = currMaxUserID + 1;
  }
  const insertResult = await appService.newUser(
    newUID,
    username,
    email,
    password,
...
router.post("/insert-cafe", async (req, res) => {
  ...
  const insertResult = await appService.insertCafe(
    currMaxCafeID + 1,
    name,
    operatingHours,
    buildingCode,
...
router.post("/insert-microwave", async (req, res) => {
  ...
  const { overallRating, buildingCode, imageURL, locationID, microwaveSize } =
    req.body;
...
router.post("/insert-review", async (req, res) => {
  ...
  if (currMaxReviewID === undefined || currMaxReviewID === null) {
    return res
      .status(400)
      .json({ success: false, message: "Failed to find suitable reviewID" });
  }

  console.log("currmaxreviewid:" + currMaxReviewID);

  const insertResult = await appService.insertReview(
    currMaxReviewID + 1,
    utilityID,
    userID,
    cleanliness,
...
router.post("/insert-request", async (req, res) => {
  ...
  const insertResult = await appService.insertRequest(
    currMaxRequestID + 1,
    requestDate,
    status,
    requestDescription,
